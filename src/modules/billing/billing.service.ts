...
export class BillingService {
  ...
  async createPackage(dto: CreatePackageDto): Promise<ZoomCreditPackage> {
    return this.prisma.zoomCreditPackage.create({
      data: {
        ...
        currency: (dto.currency ?? this.stripe.getConfig().currency).toUpperCase(),
  ...
  async updatePackage(id: string, dto: UpdatePackageDto): Promise<ZoomCreditPackage> {
    const existing = await this.prisma.zoomCreditPackage.findUnique({ where: { id } });
  ...
  async deletePackage(id: string): Promise<void> {
    ...
    await this.prisma.zoomCreditPackage.update({ where: { id }, data: { active: false } });
  }
  ...
  async updatePlan(id: string, dto: UpdateSubscriptionPlanDto) {
    ...
    return this.prisma.subscriptionPlan.update({
      ...
      data: {
        ...
        ...(dto.monthlyClassMinutes !== undefined ? { monthlyClassMinutes: dto.monthlyClassMinutes } : {}),
        ...(dto.monthlyCredits !== undefined ? { monthlyCredits: dto.monthlyCredits } : {}),
  ...
  async deletePlan(id: string): Promise<void> {
    const existing = await this.prisma.subscriptionPlan.findUnique({ where: { id } });
    if (!existing) throw new NotFoundException('Subscription plan not found.');
    await this.prisma.subscriptionPlan.update({ where: { id }, data: { active: false } });
  }
  ...
  async getMyBillingOverview(userId: string) {
    const [user, balance, subscription, payments] = await Promise.all([
      this.prisma.user.findUnique({ where: { id: userId }, select: { id: true, email: true, firstName: true, lastName: true } }),
      ...
      this.prisma.payment.findMany({ where: { userId }, orderBy: { createdAt: 'desc' }, take: 10 }),
    ]);
  ...
  async startPackageCheckout(userId: string, dto: StartCheckoutDto) {
    const pkg = await this.prisma.zoomCreditPackage.findUnique({ where: { id: dto.id } });
    if (!pkg || !pkg.active) {
      throw new NotFoundException('Package not found or inactive.');
    }
    if (pkg.billingType !== ZoomCreditPackageBillingType.ONE_TIME) {
      throw new BadRequestException('Only one-time packages can be checked out via this endpoint.');
    }
    const customer = await this.getOrCreateStripeCustomer(userId);
    const split = this.stripe.splitAmountCents(pkg.priceCents * (dto.quantity ?? 1));
    ...
    return { url: session.url, sessionId: session.id, expiresAt: session.expires_at, splits: split };
  }

  async startPlanCheckout(userId: string, dto: StartCheckoutDto) {
    const plan = await this.prisma.subscriptionPlan.findUnique({ where: { id: dto.id } });
    if (!plan || !plan.active) {
      throw new NotFoundException('Subscription plan not found or inactive.');
    }
    if (!plan.stripePriceId) {
      throw new BadRequestException('Plan is not yet provisioned with a Stripe price. Run the seed/sync first.');
    }
    const customer = await this.getOrCreateStripeCustomer(userId);
    const session = await this.stripe.createCheckoutSession({
      customerId: customer.stripeCustomerId,
      ...
      successUrl: dto.successUrl,
      cancelUrl: dto.cancelUrl,
      metadata: { userId, kind: 'subscription', planId: plan.id },
    });
    return { url: session.url, sessionId: session.id, expiresAt: session.expires_at };
  }

  async openBillingPortal(userId: string, returnUrl?: string) {
    const customer = await this.getOrCreateStripeCustomer(userId);
    const session = await this.stripe.createBillingPortalSession(customer.stripeCustomerId, returnUrl);
  ...
  async getAdminAnalytics(opts: { from?: Date; to?: Date }) {
    ...
    const [agg, byProvider, byPackage, byPlan, recent, activeSubs, mrrAgg] = await Promise.all([
    ...
    const monthlyRecurringCents = mrrAgg.reduce((acc, sub) => {
      ...
      const factor = sub.plan.interval === SubscriptionInterval.YEARLY ? 1 / 12 : 1;
  ...
  async handleStripeEvent(event: Stripe.Event): Promise<void> {
    ...
    try {
      ...
      if (err?.code === 'P2002') {
        this.logger.warn(`Duplicate webhook ${event.id} (${event.type}) - skipping.`);
        return;
      }
      throw err;
    }
    ...
    try {
      switch (event.type) {
        ...
        case 'customer.subscription.deleted':
          await this.onSubscriptionEvent(event.data.object as Stripe.Subscription);
          break;
      ...
      this.logger.error(`Failed to process webhook ${event.id} (${event.type}): ${(err as Error).message}`);
      throw err;
    }
  }
  ...
  private async onCheckoutCompleted(session: Stripe.Checkout.Session): Promise<void> {
    ...
    if (!userId) {
      this.logger.warn(`checkout.session.completed without userId: ${session.id}`);
      return;
    }
    const grossCents = session.amount_total ?? 0;
    const split = this.stripe.splitAmountCents(grossCents);
    const currency = (session.currency ?? this.stripe.getConfig().currency).toUpperCase();

    if (meta.kind === 'package' && meta.packageId) {
      const pkg = await this.prisma.zoomCreditPackage.findUnique({ where: { id: meta.packageId } });
      if (!pkg) return;
      ...
      await this.prisma.$transaction(async (tx) => {
        ...
        await tx.payment.create({
          data: {
            ...
            packageId: pkg.id,
            metadata: { stripeSessionId: session.id, paymentIntent: typeof session.payment_intent === 'string' ? session.payment_intent : session.payment_intent?.id ?? null },
          },
        });
      });
      this.logger.log(`Granted ${credits} credits to ${userId} for package ${pkg.id}`);
      return;
    }
  ...
  private async onSubscriptionEvent(stripeSub: Stripe.Subscription): Promise<void> {
    const userId = (stripeSub.metadata as Record<string, string> | null)?.userId;
    const planId = (stripeSub.metadata as Record<string, string> | null)?.planId;
    if (!userId || !planId) {
      this.logger.warn(`subscription event missing metadata.userId/planId: ${stripeSub.id}`);
      return;
    }
    const data = {
    ...
    if (existing) {
      ...
      await this.prisma.subscription.create({ data: { userId, planId, ...data } });
    }
  }

  private async onInvoicePaid(invoice: Stripe.Invoice): Promise<void> {
    if (!invoice.subscription || typeof invoice.subscription !== 'string') return;
    const sub = await this.prisma.subscription.findUnique({ where: { stripeSubscriptionId: invoice.subscription } });
    if (!sub) return;
    ...
    await this.prisma.payment.create({
      data: {
        ...
        platformFeeAmount: this.toMoney(split.fee),
        netAmount: this.toMoney(split.net),
        currency: (invoice.currency ?? this.stripe.getConfig().currency).toUpperCase(),
        provider: 'stripe',
        status: 'completed',
        reference: invoice.id,
        description: 'Subscription invoice paid',
        subscriptionId: sub.id,
        metadata: { invoiceId: invoice.id, hostedInvoiceUrl: invoice.hosted_invoice_url },
      },
    });
  }
  ...
  private async getOrCreateStripeCustomer(userId: string) {
    const existing = await this.prisma.stripeCustomer.findUnique({ where: { userId }, include: { user: true } });
